...
export default function AdminDashboard() {
  ...
  const loadGlobalData = async () => {
    // 1. Stats Setup (Today's Confirmed Bookings)
    try {
      ...
      let { data: bData } = await supabase
        .from('bookings')
        .select('*, halls(name), profiles(email)')
        .order('created_at', { ascending: false });

      // Fallback if profiles(email) relational join fails natively
      if (!bData) {
         const fallbackData = await supabase.from('bookings').select('*, halls(name)').order('created_at', { ascending: false });
         bData = fallbackData.data || [];
      }
      setBookings(bData || []);

      // 3. Fetch Halls
      const { data: hData } = await supabase.from('halls').select('*').order('created_at', { ascending: true });
      setHalls(hData || []);

      // 4. Fetch Profiles precisely for Broadcast Emails mapping
      const { data: pData } = await supabase.from('profiles').select('email');
      setProfiles(pData || []);

    } catch (err) {
      console.error("Dashboard Load Error:", err);
      alert("⚠️ التنبيه الأمني الأول: الجداول فارغة لأن صلاحيات Supabase (RLS) تمنع الآدمن من رؤية الجداول كاملة. يرجى تفعيل سياسات SELECT.");
    }
  ...
  const saveHallDb = async () => {
    setIsUploading(true);
    let imageUrl = editingHall?.image || "";

    // Storage Supabase Logic Upload Engine
    if (hallFile) {
       ...
       const { data: uploadData, error: uploadErr } = await supabase.storage
         .from('hall-images')
         .upload(fileName, hallFile, { cacheControl: '3600', upsert: false });

       if (uploadErr) {
         console.error("Storage Upload Error:", uploadErr);
         alert("خطأ في رفع الصورة، يُرجى التأكد من سياسات البوكيت public.");
    ...
    let dbError = null;
    if (editingHall) {
      const resp = await supabase.from('halls').update(payload).eq('id', editingHall.id).select();
      dbError = resp.error || (!resp.data || resp.data.length === 0 ? "RLS Blocked Update" : null);
    } else {
      const resp = await supabase.from('halls').insert(payload).select();
      dbError = resp.error || (!resp.data || resp.data.length === 0 ? "RLS Blocked Insert" : null);
    }

    if (dbError) {
      console.error("DB Save Error:", dbError);
      alert("⚠️ فشل الحفظ في قاعدة البيانات. هذا بنسبة 100% خاضع لسياسات RLS. تأكد من إنشاء سياسة INSERT/UPDATE في Supabase.");
    } else {
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {halls.map((h, i) => (
                  <tr key={h.id} className="hover:bg-gray-50 transition-colors">
                    <td className="p-3">
                      <img src={h.image} alt="hall" className="w-14 h-14 object-cover rounded-lg border border-gray-300 shadow-sm" onError={(e)=>{e.target.src='https://via.placeholder.com/150'}}/>
                    </td>
                    <td className="p-4 font-black text-[#1A365D]">{h.name}</td>
                    <td className="p-4 font-bold text-gray-600">{h.location}</td>
                    <td className="p-4 font-black text-[#C8A97E]">{h.price}</td>
                    <td className="p-4">
                      <div className="flex justify-center items-center gap-3">
                        <button onClick={() => openHallModal(h)} className="p-2bg-blue-100 text-blue-600 hover:bg-blue-200 rounded-lg p-2 transition-colors" title="تعديل">
                          <Edit2 className="w-5 h-5" />
                        </button>
                        <button onClick={() => deleteHallSync(h.id)} className="p-2bg-red-100 text-red-600 hover:bg-red-200 rounded-lg p-2 transition-colors" title="حذف بالكامل">
